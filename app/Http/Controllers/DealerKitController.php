...
// Replace the section starting from "if (count($allVehicles) > 0) {" 
// with this corrected version:

if (count($allVehicles) > 0) {
    // Get all cars for this dealer that are currently active
    $activeCars = Car::whereIn('advert_id', function ($query) use ($user) {
    ...
    // Get sample of database vehicle IDs to compare format
    $dbVehicleIds = $activeCars->pluck('vehicle_id')->take(5)->toArray();


    $carsToMarkAsSold = [];
    
    foreach ($activeCars as $car) {
        // If this car's vehicle_id is NOT in the API feed, it's been sold
    ...
    foreach ($carsToMarkAsSold as $carInfo) {
        try {
            ...
            foreach ($allVehicles as $vehicleData) {
                ...
                $existingCar = Car::where('vehicle_id', $vehicleData['id'])->first();
                if ($existingCar) {
